...
class RewarderTool {
  ...
  Future<bool> isAdReady() async {
    try {
      final isReady = await ATRewardedManager.rewardedVideoReady(
        placementID: placementId,
      );
      print('✅ Rewarded ad ready: $isReady');
      return isReady;
    } catch (e) {
      print('❌ Error checking ad readiness: $e');
      return false;
    }
  }
  ...
  void _handleAdEvent(value) {
    ...
    switch (value.rewardStatus) {
      ...
      case RewardedStatus.rewardedVideoDidEndPlaying:
        log(
          "flutter rewardedVideoDidEndPlaying ---- placementID: ${value.placementID}",
        );
        break;

      // This is for repeat plays (optional)
      case RewardedStatus.rewardedVideoDidAgainEndPlaying:
        log(
          "flutter rewardedVideoDidAgainEndPlaying ---- placementID: ${value.placementID}",
        );
        break;
      case RewardedStatus.rewardedVideoDidClose:
        log("flutter DID CLOSE---- placementID: ${value.placementID}");
        AdTimerService().onAdEnded(); // Also start timer if needed
        break;
    }
  }
